...
$report_response = api_get('/api/reports/profit-loss?year=' . $year . '&month=' . $month);
$report = $report_response['success'] ? $report_response['report'] : array();
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        ...
        .sidebar { position: fixed; top: 0; left: 0; width: var(--sidebar-width); height: 100vh; background: var(--primary); border-right: 1px solid var(--border); z-index: 1000; overflow-y: auto; }
        ...
        .table-card .table thead th { background: rgba(255,255,255,0.03); border-bottom: 1px solid var(--border); color: #8b949e; font-weight: 600; font-size: 0.8rem; text-transform: uppercase; padding: 0.85rem 1rem; }
        .table-card .table tbody td { border-bottom: 1px solid var(--border); padding: 0.75rem 1rem; }
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="p-3 p-md-4">
            <?php if ($flash): ?>
            <div class="alert <?php echo alert_class($flash['type']); ?> alert-dismissible fade show">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    ...
                    <p class="text-muted mb-0">Financial summary for <?php echo date('F Y', mktime(0, 0, 0, $month, 1, $year)); ?></p>
                </div>
                <form method="GET" class="d-flex gap-2">
                    <select name="month" class="form-control form-control-sm" style="width: auto;">
                        <?php for ($m = 1; $m <= 12; $m++): ?>
                        <option value="<?php echo $m; ?>" <?php echo $m == $month ? 'selected' : ''; ?>><?php echo date('F', mktime(0, 0, 0, $m, 1)); ?></option>
                        <?php endfor; ?>
                    </select>
                    <select name="year" class="form-control form-control-sm" style="width: auto;">
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-4">
                    <div class="stat-card">
                        <div class="value" style="color: #3fb950;"><?php echo format_money(isset($report['total_income']) ? $report['total_income'] : 0); ?></div>
                        <div class="label">Total Income</div>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="stat-card">
                        <div class="value" style="color: #f85149;"><?php echo format_money(isset($report['total_expenses']) ? $report['total_expenses'] : 0); ?></div>
                        <div class="label">Total Expenses</div>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="stat-card">
                        <div class="value" style="color: #58a6ff;"><?php echo format_money(isset($report['net_profit']) ? $report['net_profit'] : 0); ?></div>
            ...
            <div class="table-card">
                ...
                <div class="table-responsive">
                    <table class="table">
                        ...
                        <tbody>
                            <?php foreach ($report['transactions'] as $t): ?>
                            <tr>
                                <td><?php echo format_date($t['date']); ?></td>
                                <td><?php echo h($t['description']); ?></td>
                                <td><?php echo h($t['type']); ?></td>
                                <td><?php echo format_money($t['amount']); ?></td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
